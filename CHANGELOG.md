...
Test technique isolé, désactivé par défaut (`COMPOSIO_POC_ENABLED=false` → EMA inchangé). Client REST v3.1 (`src/integrations/composio/client.ts`, URL officielle `https://backend.composio.dev`, SDK écarté : Node ≥ 22.22.3 requis), politique lecture seule fail-closed avec **table déterministe de slugs Outlook actuels** (`policy.ts`), service par utilisateur (`outlook-poc.ts`), migration `011_composio_poc` (références uniquement, aucun token), routes `/api/poc/composio/*`, page `/poc/composio`. Corrections d'audit : `input_parameters` v3.1 (mapping direct) + forme JSON Schema, slugs `OUTLOOK_*` actuels (jamais un tool de calendrier pour une pièce jointe de message), **Callback Identity Verification** (`complete_auth`, obligatoire en production, mode callback local toléré en développement seulement), auth config Composio Managed OAuth lecture seule documentée (scopes séparés par des virgules), callback fail-closed sur l'état local (refus sans connexion en attente, révocation best-effort d'un compte activé étranger, modes de route explicites). Préparation du smoke test réel : **preflight** serveur (`preflight.ts`, `GET /api/poc/composio/preflight`, authentifié, présence des variables sans jamais leur valeur, mode de retour, sécurité production, `APP_URL`, URL Composio, base et migration `011`, Graph natif intact, 8 tools attendus, capacités `email.read` / `email.search` / `attachment.read` / `calendar.read`), **smoke test guidé** en 12 étapes (PAS TESTÉ / OK / ÉCHEC, états uniquement dans l'onglet, rien en base), résumé de diagnostic **`WRITE tools executable = 0`** (sinon POC en échec), checklist « Avant le test réel » et procédure « Test réel Outlook 3T ». 40 tests mockés (réponses v3.1, faux secrets uniquement). Procédure : `docs/composio-poc.md`.
